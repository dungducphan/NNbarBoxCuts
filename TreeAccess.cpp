...
TreeAccess::TreeAccess(std::string fileName, TTree *tree)
    ...
    h2D_AverageEnergyPerHit = new TH2D("", "", 1, 0, 1, 1, 0, 1);
...
//    h_AverageEnergyPerHit = new TH2D("AverageEnergyPerHit", ";Energy per Hit X-view (GeV);Energy per Hit Y-view (GeV)", 100, 0, 0.06, 100, 0, 0.06);
//    h_AreaRatio = new TH2D("AreaRatio", ";Area Ratio X-view;Area Ratio Y-view", 100, 0, 1, 100, 0, 1);
//
//    TH2DMaps["AverageEnergyPerHit"] = h_AverageEnergyPerHit;
//    TH2DMaps["AreaRatio"] = h_AreaRatio;
}
...
void TreeAccess::Terminate2D(std::string histName) {
    delete TH2DMaps[histName.c_str()];
}
...
TH2D *TreeAccess::GetTH2DByName(std::string histName) {
    return TH2DMaps[histName.c_str()];
}
...
bool TreeAccess::PassVisECut() {
    return ((TotalVisibleEnergy >= 0.6) && (TotalVisibleEnergy <= 1.6));
}

bool TreeAccess::PassAvgEPerHitYViewCut() {
    return (AverageEnergyPerHitYView >= 0.0125);
}
...
bool TreeAccess::PassAvgYPositionCut() {
    return (AverageYposition <= 100);
}

bool TreeAccess::PassTotHitCountXViewCut() {
    return (TotalHitCountInXView <= 40);
}

bool TreeAccess::PassTotHitCountYViewCut() {
    return (TotalHitCountInYView <= 40);
}

bool TreeAccess::PassAreaRatioXViewCut() {
    return ((ConvexHullAreaXV/MinimalEnclosingCircleAreaXV) >= 0.15);
}

bool TreeAccess::PassAreaRatioYViewCut() {
    return ((ConvexHullAreaYV/MinimalEnclosingCircleAreaYV) >= 0.15);
}
...
//        if (fAvgEPerHitXViewCut && !PassAvgEPerHitXViewCut()) continue;
        if (fAvgEPerHitCompositeCut && !PassAvgEPerHitCompositeCut()) continue;
        ...
        h2D_AverageEnergyPerHit->Fill(AverageEnergyPerHitXView, AverageEnergyPerHitYView);
        h2D_AreaRatio->Fill(ConvexHullAreaXV / MinimalEnclosingCircleAreaXV, ConvexHullAreaYV / MinimalEnclosingCircleAreaYV);
    }
