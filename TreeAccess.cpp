...
TreeAccess::TreeAccess(std::string fileName, TTree *tree)
  ...
  h_AverageEnergyPerHit = new TH2D("AverageEnergyPerHit", ";Energy per Hit X-view (GeV);Energy per Hit Y-view (GeV)",
                                   100, 0, 0.06, 100, 0, 0.06);
...
bool TreeAccess::PassVisECut() {
  return ((TotalVisibleEnergy >= 0.8) && (TotalVisibleEnergy <= 1.5));
}
...
bool TreeAccess::PassAvgEPerHitXViewCut() {
  return (AverageEnergyPerHitXView >= 0.015);
}

bool TreeAccess::PassAvgYPositionCut() {
  return (AverageYposition <= 200);
}
...
void TreeAccess::Loop() {
  ...
  for (Long64_t jentry = 0; jentry < nentries; jentry++) {
    ...
    if (fIsCosmic) {
      std::cout << "Run #" << RunNumber << ". Subrun #" << SubrunNumber << ". Event #" << EventNumber << ". Slice #" << SliceNumber << "." << std::endl;
    }
    ...
    h_AverageEnergyPerHit->Fill(AverageEnergyPerHitXView, AverageEnergyPerHitYView);
    h_AreaRatio->Fill(ConvexHullAreaXV / MinimalEnclosingCircleAreaXV, ConvexHullAreaYV / MinimalEnclosingCircleAreaYV);
  }
